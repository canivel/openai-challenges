...
def load_data_shard(file):
	header_bytes=256*np.dtype(_R).itemsize;token_bytes=np.dtype(_X).itemsize;header=np.fromfile(file,dtype=_R,count=256)
...
_SHARD_HEADER_BYTES=256*np.dtype(_R).itemsize
...
def _read_num_tokens(file):
	...
	header=np.fromfile(file,dtype=_R,count=256)
	if header.size!=256 or int(header[0])!=20240520 or int(header[1])!=1:raise ValueError(f"Unexpected shard header for {file}")
...
def classify_param(name):
	...
	if'.attn.'in name or'.proj.'in name and A not in name:return _S
	return'other'
@torch.compile
...
def collect_hessians(model,train_loader,h,device,n_calibration_batches=64):
	...
	for(name,module)in model.named_modules():
		if isinstance(module,CastedLinear)and module.weight.numel()>65536:
			...
			if cat in(_O,_S):hooks.append(module.register_forward_hook(make_hook(name+A)))
	model.eval()
	with torch.no_grad():
...
def gptq_mixed_quantize_int6(state_dict,int6_cats,hessians,clip_ranges=_A):
	...
	for(name,tensor)in state_dict.items():
		...
		if cat in int6_cats and t.ndim==2:
			bit_label=B if layer_clip==15 else _T
		...
		elif cat in int6_cats and t.ndim>=1:q,s=quantize_int6_per_row(t,clip_range=layer_clip);bit_label=B if layer_clip==15 else _T;result[name+_K]=q;result[name+_L]=s;meta[name]={_J:bit_label,A:layer_clip}
...
def mixed_quantize_int6(state_dict,int6_cats):
	...
	for(name,tensor)in state_dict.items():
		...
		if not t.is_floating_point()or t.numel()<=65536:result[name]=t.to(torch.float16)if t.is_floating_point()else t;meta[name]=_b;continue
		if any(p in name for p in CONTROL_TENSOR_NAME_PATTERNS):result[name]=t.float();meta[name]=_c;continue
		if cat in int6_cats and t.ndim>=1:q,s=quantize_int6_per_row(t);result[name+_K]=q;result[name+_L]=s;meta[name]={_J:_T}
		else:q,s=quantize_float_tensor(t);result[name+_K]=q;result[name+_L]=s;meta[name]={_J:'int8'}
	return result,meta
...
def serialize(h,base_model,code):
	...
	if h.gptq_enabled:
		log('GPTQ:collecting Hessians from calibration data...');t0=time.perf_counter();calib_loader=DistributedTokenLoader(h.train_files,h.rank,h.world_size,torch.device(_M,h.local_rank));hessians=collect_hessians(base_model,calib_loader,h,torch.device(_M,h.local_rank),n_calibration_batches=h.gptq_calibration_batches);log(f"GPTQ:collected {len(hessians)} Hessians in {time.perf_counter()-t0:.1f}s");clip_ranges=_A
		if h.mixed_quant:
			sensitivity={}
		...
		quant_result,quant_meta=gptq_mixed_quantize_int6(sd_cpu,{_O,_S},hessians,clip_ranges=clip_ranges)
	else:quant_result,quant_meta=mixed_quantize_int6(sd_cpu,{_O,_S})
	...
	if h.is_main_process:
		...
		quant_label='mixed_int5_int6'if h.gptq_enabled and h.mixed_quant else _T;log(f"Serialized model {quant_label}+{h.compressor}: {quant_file_bytes} bytes");log(f"Total submission size {quant_label}+{h.compressor}: {bytes_total} bytes")
...
def train_and_eval(h,device):
	random.seed(h.seed);np.random.seed(h.seed);torch.manual_seed(h.seed);torch.cuda.manual_seed_all(h.seed);val_data=ValidationData(h,device);log(f"train_shards: {len(list(Path(h.datasets_dir).resolve().glob(_f)))}");log(f"val_tokens: {val_data.val_tokens.numel()-1}");base_model,compiled_model=train_model(h,device,val_data);timed_eval('pre-quantization post-ema',eval_val,h,device,val_data,compiled_model);serialize(h,base_model,Path(__file__).read_text(encoding=_N))
	if h.distributed:dist.barrier()
	eval_model=deserialize(h,device);run_evals(h,device,val_data,eval_model)
def main():
	A='=';world_size=int(os.environ.get(_Q,_E));local_rank=int(os.environ.get(_e,'0'));distributed=_W in os.environ and _Q in os.environ
